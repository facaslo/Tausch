...
const acceptOffer = async (req) => {
    ...
    try{
        ...
        for (const idPub of req.body.idsPublications){
            await client_pool.query(`UPDATE propuesta SET estado_propuesta='cancelada' WHERE estado_propuesta='en espera' AND id_publicacion_receptor='${idPub}' and id_propuesta !='${req.body.idOffer}';`).then(res => res.rows).catch(e => console.log(e))
            await client_pool.query(`UPDATE propuesta SET estado_propuesta='cancelada' WHERE estado_propuesta='en espera' AND id_propuesta in (select id_propuesta from propuesta_tiene_publicacion where id_publicacion='${idPub}') and id_propuesta !='${req.body.idOffer}';`).then(res => res.rows).catch(e => console.log(e))
